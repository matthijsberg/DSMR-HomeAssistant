// Modified version based on this: https://github.com/nldroid/CustomP1UartComponent
#include "esphome.h"
#include "dsmr.h"
...
using MyData = ParsedData <
	...
	/* StringValue */ electricity_tariff,
	/* FixedValue */ power_delivered,
	/* FixedValue */ power_returned,
	/* uint32_t */ electricity_failures,
	/* uint32_t */ electricity_long_failures,
	/* StringValue */ electricity_failure_log,
...
void publish_sensors(MyData data){
	...
	if(data.electricity_failures_present)s_electricity_failures->publish_state(data.electricity_failures);/* NEW */
	if(data.electricity_long_failures_present)s_electricity_long_failures->publish_state(data.electricity_long_failures);/* NEW */
	if(data.electricity_sags_l1_present)s_electricity_sags_l1->publish_state(data.electricity_sags_l1);/* NEW */
	if(data.electricity_sags_l2_present)s_electricity_sags_l2->publish_state(data.electricity_sags_l2);/* NEW */
	if(data.electricity_sags_l3_present)s_electricity_sags_l3->publish_state(data.electricity_sags_l3);/* NEW */
	if(data.electricity_swells_l1_present)s_electricity_swells_l1->publish_state(data.electricity_swells_l1);/* NEW */
	if(data.electricity_swells_l2_present)s_electricity_swells_l2->publish_state(data.electricity_swells_l2);/* NEW */
	if(data.electricity_swells_l3_present)s_electricity_swells_l3->publish_state(data.electricity_swells_l3);/* NEW */
...
public:
	...
	Sensor *s_electricity_sags_l2 = new Sensor();
	Sensor *s_electricity_sags_l3 = new Sensor();
	Sensor *s_electricity_swells_l1 = new Sensor();
	Sensor *s_electricity_swells_l2 = new Sensor();
	Sensor *s_electricity_swells_l3 = new Sensor();
	//Sensor *s_electricity_threshold = new Sensor(); removed in 5
